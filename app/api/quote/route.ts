...
function buildAdminHtml(body: QuotePayload) {
  const {
    name, company, email, phone, serviceType, eventDates, location,
    venueEnvironment, screenWidth, screenHeight, numberOfScreens,
    contentType, pixelPitchPreference, closestViewerDistance,
    budgetRange, targetCompletionDate, additionalNotes, heardAbout,
  } = body;

  const projectTypeLabel = serviceType === "rentals" ? "Rental" : "Installation";
  const eventDatesRow = serviceType === "rentals" ? safe(eventDates) : "N/A";
  const screenDimensionsRow = `${safe(screenWidth)} x ${safe(screenHeight)}`;

  const labelStyle = "padding: 4px 0; font-size: 12px; letter-spacing: 0.16em; text-transform: uppercase; color: #C8A96E;";
  const valueStyle = "font-size: 14px; color: #ffffff;";

  const row = (label: string, value: string | undefined | null) => `
    <tr><td style="${labelStyle}">${label}</td></tr>
    <tr><td style="${valueStyle}; padding-bottom: 12px;">${safe(value)}</td></tr>
  `;
      ...
      <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="background-color:#060606; padding:32px 16px;">
        <tr><td align="center">
          <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="max-width:640px; border:1px solid #1b1b1b; background:radial-gradient(circle at top, #151515 0, #060606 48%, #000000 100%); padding:32px 28px 28px; border-radius:8px;">
            <tr><td style="padding-bottom:24px; border-bottom:1px solid #202020;">
              <div style="font-size:11px; letter-spacing:0.32em; text-transform:uppercase; color:#C8A96E; margin-bottom:8px;">New Quote Request</div>
              <div style="font-size:24px; font-weight:600; text-transform:uppercase; letter-spacing:0.12em;">${BRAND_NAME} · LED Display Solutions</div>
              <div style="margin-top:8px; font-size:13px; color:#b3b3b3;">Submitted from fetanled.com contact form.</div>
            </td></tr>
            <tr><td style="padding-top:24px;">
              <table width="100%" cellpadding="0" cellspacing="0" role="presentation">
                ${row("Full Name", name)}
                ${row("Company / Organisation", company)}
                ${row("Project Type (Installation or Rental)", projectTypeLabel)}
      ...
      <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="background-color:#060606; padding:32px 16px;">
        <tr><td align="center">
          <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="max-width:620px; border:1px solid #1b1b1b; background:radial-gradient(circle at top, #151515 0, #060606 48%, #000000 100%); padding:32px 28px 28px; border-radius:8px;">
            <tr><td>
              <div style="font-size:11px; letter-spacing:0.32em; text-transform:uppercase; color:#C8A96E; margin-bottom:8px;">Quote Request Received</div>
              <div style="font-size:22px; font-weight:600; text-transform:uppercase; letter-spacing:0.16em; margin-bottom:16px;">Thank You${name ? `, ${name}` : ""}.</div>
              <p style="font-size:14px; color:#e0e0e0; line-height:1.7; margin:0 0 12px;">
                We've received your quote request for LED display solutions at <strong>${BRAND_NAME}</strong>${company ? ` on behalf of <strong>${company}</strong>` : ""}.
              </p>
              <p style="font-size:14px; color:#cccccc; line-height:1.7; margin:0 0 12px;">
                A member of our team will review your project details and get back to you within <strong>4 hours</strong> with next steps and any clarifying questions.
              </p>
              ...
              <p style="font-size:13px; color:#777777; line-height:1.6; margin:20px 0 0;">— The ${BRAND_NAME} Team</p>
            </td></tr>
          </table>
        </td></tr>
      </table>
...
export async function POST(req: Request) {
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ error: "Unexpected error while submitting quote. Please try again." }),
      { status: 500 }
    );
  }
}
