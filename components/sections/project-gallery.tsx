...
function ProjectCard({ project, index }: { project: (typeof PROJECTS)[number]; index: number }) {
  ...
  return (
    <motion.article
      ...
      className={cn(
        "group relative overflow-hidden border border-white/10 bg-card-gradient transition-all duration-500 hover:border-brand-blue/50 hover:shadow-[0_0_40px_rgba(29,116,255,0.22)]",
        isLarge
    ...
    >
      ...
      <div className="relative flex h-full flex-col justify-between p-5 sm:p-6">
        <div className="flex items-start justify-between">
          ...
          {project.size && (
            <span className="font-display text-lg font-bold text-white/80 sm:text-xl">{project.size}</span>
          )}
        </div>
        <div>
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <motion.div layout className="mt-10 grid auto-rows-fr grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
